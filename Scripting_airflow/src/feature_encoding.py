...
class NominalEncodingStrategy(FeatureEncodingStrategy):
    ...
    def encode(self, df):
        ...
        for col in self.nominal_columns:
            ...
            # Step 3: Create individual binary columns directly from the vector
            # Extract each element of the vector as a separate column using vector_to_array
            from pyspark.ml.functions import vector_to_array
            
            df_encoded = df_encoded.withColumn("tmp_array", vector_to_array(F.col(f"{col}_vec")))
